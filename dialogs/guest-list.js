    ...
    function (session, results) {
        if (results.response && results.response.validCode == true) {
            ...
            createOrder(params, function(statusCode) {
                if(statusCode == 200) {
                    session.endConversation(`You have now been successfully guest listed for ${session.dialogData.event} at ${session.dialogData.venue}! Your name will be under ${session.dialogData.promoter.name.first} ${session.dialogData.promoter.name.last} so please bring a valid ID with birth date.\n
